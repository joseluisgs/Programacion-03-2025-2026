...
do {
    ...
    CalcularSiguienteEstado(tableroFront, tableroBack);
...
MostrarResultados(tableroFront, ciclosTranscurridos);
 ...
 * Recorre el tablero de lectura (T) y aplica las modificaciones al tablero de escritura (T+1).
 */
void CalcularSiguienteEstado(TipoCelda[,] tableroLectura, TipoCelda[,] tableroEscritura) {
        ...
        for (var j = 0; j < dimensionLocal; j++) {
            ...
            if (tipoCelda == TipoCelda.Infectada) {
                ...
                if (RealizarSorteo(configuracion.Muerte))
                    ...
                    MoverEInfectar(tableroLectura, tableroEscritura, i, j); // Zombie se mueve/infecta
            }
                ...
                MoverSano(tableroLectura, tableroEscritura, i, j);
            }
        }
    }
}
 ...
 * tiempo El número total de CICLOS transcurridos en la simulación.
 */
void MostrarResultados(TipoCelda[,] tablero, int tiempo) {
    Console.Clear();
